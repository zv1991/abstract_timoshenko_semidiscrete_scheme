...
# ======================================================
# CLASS: Testcase13 – Trigonometric Benchmark for Timoshenko System
# ======================================================

class Testcase13(TimoshenkoTesterParent):
    """
    ...
    def __init__(self, cfg):
        ...
        """
        self.cfg = cfg
        self.name = "test13"            # Identifier for file/log output
        ...
        self.d  = self.cfg.lam * np.pi / self.cfg.ell
        self.d1 = self.cfg.lam1 * np.pi / self.cfg.T
    ...
    def h_u(self, x):
        ...
        """
        return self.cfg.A * np.sin(self.d * x)

    def d1h_u(self, x):
        """
        First derivative of h_u(x) with respect to x (i.e., ∂h_u/∂x).
        """
        return self.cfg.A * self.d * np.cos(self.d * x)
    ...
    def g_u(self, t):
        """
        Temporal basis for displacement.
        """
        return np.cos(self.d1 * t)

    def d2g_u(self, t):
        """
        Second derivative of g_u(t) — ∂²g_u/∂t² for use in acceleration terms.
        """
        return -self.d1**2 * np.cos(self.d1 * t)
    ...
    def nonlinear_term(self, t):
        ...
        integr_coeff = (
            (self.cfg.A * self.cfg.lam * np.pi)**2 / (2.0 * self.cfg.ell)
            )
